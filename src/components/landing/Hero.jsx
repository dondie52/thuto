  ...
  return (
    <section className="relative isolate flex min-h-[100svh] flex-col justify-end overflow-hidden bg-brand-950 pb-14 pt-28 sm:pb-20 sm:pt-32">
      ...
      <div className="relative z-10 mx-auto w-full max-w-6xl px-4 sm:px-6">
        <p className="landing-hero-kicker text-xs font-semibold uppercase tracking-[0.28em] text-amber-300 sm:text-sm">
          {content?.kicker}
        </p>
        <h1
        ...
        >
          {content?.title}
          {content?.titleEmphasis ? <> <em className="text-amber-400 italic">{content.titleEmphasis}</em></> : null}
        </h1>
        ...
        <div className="landing-hero-actions mt-8 flex flex-wrap items-center gap-3 sm:mt-10">
          <Link
            to="/predictor"
            className="focus-ring-on-dark landing-motion-press inline-flex min-h-[44px] min-w-[44px] items-center justify-center rounded-md bg-amber-500 px-7 py-3.5 text-sm font-semibold text-slate-950 shadow-lg hover:bg-amber-400 hover:shadow-xl"
          >
            {content?.primaryCtaLabel}
          </Link>
          <Link
            to={landingTo(isSignedIn, "/programmes", "#programmes")}
            className="focus-ring-on-dark landing-motion-press inline-flex min-h-[44px] items-center justify-center rounded-md border border-white/30 bg-white/10 px-6 py-3.5 text-sm font-semibold text-white shadow-sm backdrop-blur-sm hover:border-white/50 hover:bg-white/20"
          >
