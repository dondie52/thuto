...
const features = [
  {
    ...
    body: "Start with real or estimated BGCSE grades and see programmes you may qualify for.",
    to: "/predictor",
    guestHash: "#features",
    icon: (
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <LandingReveal className="mb-12 max-w-2xl">
          ...
          <p className="mt-3 text-base leading-relaxed text-slate-600">
            Instead of opening multiple university websites and guessing where you qualify, check your eligibility first and
            use that result to build a clearer shortlist.
          </p>
        </LandingReveal>
        <LandingReveal
          as="h2"
        ...
        <ul className="mt-10 grid gap-5 sm:grid-cols-2">
          {features.map((f, index) => {
            const href = f.to === "/predictor" ? "/predictor" : landingTo(isSignedIn, f.to, f.guestHash);
            return (
              <LandingReveal as="li" key={f.title} delay={index * 90}>
                ...
                >
                  ...
                  <span className="mt-4 text-sm font-semibold text-brand-700">
                    {f.to === "/predictor" ? "Check eligibility ->" : isSignedIn ? "Open in app ->" : "Learn more ->"}
                  </span>
                </Link>
              </LandingReveal>
            );
